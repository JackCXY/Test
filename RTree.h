...
public:
	...
	int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], vector<int> &vr);
...
protected:
	...
	bool Search(Node* a_node, Rect* a_rect, vector<int> &vr);
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], vector<int> &vr)
{
#ifdef _DEBUG
	for (int index = 0; index<NUMDIMS; ++index)
	{
...
bool RTREE_QUAL::LoadRec(Node* a_node, RTFileStream& a_stream)
{
	...
	if (a_node->IsInternalNode())  // not a leaf node
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			//以在内存中分配的地址为指针地址，因为必须全部加载进入内存中，不具有很好的移植性
			...
			if (Overlap(a_rect, &(a_node->m_branch[index].m_rect)))
			{
				if (!RemoveRectRec(a_rect, a_id, a_node->m_branch[index].m_child, a_listNode))
				{
					...
					else
					{
						...
						//update the up down information
						//传递地址值，然后更新数据块内容信息
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, vector<int> &vr)
{
	ASSERT(a_node);
	ASSERT(a_node->m_level >= 0);
	ASSERT(a_rect);
	...
	if (a_node->IsInternalNode()) // This is an internal node in the tree
	{
		for (int index = 0; index < a_node->m_count; ++index)
		{
			if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
			{
				if (!Search(a_node->m_branch[index].m_child, a_rect, vr))
				{
